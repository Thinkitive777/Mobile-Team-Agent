...
// One-time migration from the pre-rename config directory. Done per item rather
// than by moving the whole directory, because the installer creates CONFIG_DIR
// before the server first runs — a directory-level check would never fire.
// An item is only moved when the destination does not already exist, so a live
// config is never clobbered. `src/` and `bin/` are deliberately excluded: those
// are the installed program, which the installer writes fresh.
if (fs.existsSync(CONST.LEGACY_CONFIG_DIR)) {
  const migrated = [];
  for (const item of CONST.LEGACY_MIGRATION_ITEMS) {
    const from = path.join(CONST.LEGACY_CONFIG_DIR, item);
    const to = path.join(CONST.CONFIG_DIR, item);
    if (!fs.existsSync(from) || fs.existsSync(to)) continue;
    try {
      fs.renameSync(from, to);
      migrated.push(item);
    } catch (error) {
      // Non-fatal — the agent still works, the user just reconfigures.
      Logger.warn("Could not migrate legacy config item", { item, error: error.message });
    }
  }
  if (migrated.length > 0) {
    Logger.info("Migrated config from pre-rename directory", {
      from: CONST.LEGACY_CONFIG_DIR,
      to: CONST.CONFIG_DIR,
      items: migrated,
    });
  }
