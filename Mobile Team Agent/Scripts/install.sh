...
# ============================================================
# Mobile Team Agent — Global Installer (Mac / Linux)
#
# Usage: chmod +x install.sh && ./install.sh
#
# What this does:
#   1. Installs the agent binary to ~/.mobile-team-agent/
...
print_banner() {
    echo ""
    echo -e "${CYAN}============================================${NC}"
    echo -e "${CYAN}  Mobile Team Agent Installer v${VERSION}${NC}"
    echo -e "${CYAN}============================================${NC}"
    echo ""
}
...
case "$OS_TYPE" in
    Darwin)
        OS_LABEL="macOS"
        SOURCE_BINARY="$SCRIPT_DIR/dist/mobile-team-agent-macos"
        ;;
    Linux)
        OS_LABEL="Linux"
        SOURCE_BINARY="$SCRIPT_DIR/dist/mobile-team-agent-linux"
        ;;
    *)
...
#!/bin/bash
AGENT_DIR="$HOME/.mobile-team-agent/src"
...
if [ -z "$INSTALL_MODE" ]; then
    ...
    echo "    2. Build binaries: cd 'Mobile Team Agent' && npm run build"
    exit 1
fi
...
# Remove the pre-rename registration. Without this an upgrading install keeps a
# second server named 'projectguide-agent' pointing at the old directory, which
# exposes a duplicate copy of every tool.
...
if [ "$INSTALL_MODE" = "source" ]; then
    ...
    if claude mcp add mobile-team-agent -s user -- $MCP_CMD "$MCP_ARGS" 2>&1; then
    ...
    MCP_ARGS=""
    if claude mcp add mobile-team-agent -s user -- "$MCP_CMD" 2>&1; then
...
if [ -f "$CLAUDE_JSON" ]; then
    if grep -q "mobile-team-agent" "$CLAUDE_JSON" 2>/dev/null; then
        MCP_REGISTERED=true
    fi
fi
...
MCPEOF
    ...
    if [ "$MCP_REGISTERED" = true ]; then
        ...
        if [ "$INSTALL_MODE" = "source" ]; then
            echo "    claude mcp add mobile-team-agent -s user -- node $INSTALL_DIR/src/Main/index.js"
        else
...
if [ -n "$CLAUDE_MD_SOURCE" ]; then
    ...
    MARKER="# --- Mobile Team Agent Instructions ---"
    MARKER_END="# --- End Mobile Team Agent Instructions ---"
...
if [ -n "$SHELL_RC" ]; then
    if ! grep -q "mobile-team-agent/bin" "$SHELL_RC" 2>/dev/null; then
        echo "" >> "$SHELL_RC"
        echo "# Mobile Team Agent" >> "$SHELL_RC"
