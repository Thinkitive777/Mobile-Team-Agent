...
:: ============================================================
:: Mobile Team Agent — Global Installer (Windows)
::
:: Usage: Right-click > Run as Administrator, or just double-click
::
:: What this does:
::   1. Installs the agent to %USERPROFILE%\.mobile-team-agent\
...
set "BINARY_NAME=mobile-team-agent.exe"
...
echo.
echo ============================================
echo   Mobile Team Agent Installer v%VERSION%
echo ============================================
echo.
...
:: ----------------------------------------------------------
set "INSTALL_MODE="
set "SOURCE_BINARY=%SCRIPT_DIR%dist\mobile-team-agent-win.exe"
...
:: Remove the pre-rename registration so upgrading installs don't keep a
:: duplicate 'projectguide-agent' server exposing the same tools.
...
if "%INSTALL_MODE%"=="binary" (
    claude mcp add mobile-team-agent -s user -- "%BIN_DIR%\%BINARY_NAME%"
) else (
    claude mcp add mobile-team-agent -s user -- node "%INSTALL_DIR%\src\Main\index.js"
)

:: Verify registration in ~/.claude.json
set "MCP_REGISTERED=0"
set "CLAUDE_JSON=%USERPROFILE%\.claude.json"
findstr /C:"mobile-team-agent" "%CLAUDE_JSON%" >nul 2>&1
if !ERRORLEVEL! equ 0 (
...
if "!MCP_REGISTERED!"=="0" (
    ...
    powershell -Command ^
        ...
        "}; " ^
        "$config.mcpServers | Add-Member -NotePropertyName 'mobile-team-agent' -NotePropertyValue $server -Force; " ^
    ...
    if !ERRORLEVEL! equ 0 (
        ...
        echo         claude mcp add mobile-team-agent -s user -- "%BIN_DIR%\%BINARY_NAME%"
    )
)
...
if defined CLAUDE_MD_SOURCE (
    ...
    findstr /C:"Mobile Team Agent Instructions" "!GLOBAL_CLAUDE_MD!" >nul 2>&1
    if !ERRORLEVEL! equ 0 (
        echo [INFO]  Agent instructions already in global CLAUDE.md. Updating...
        :: Create temp file without our block, then re-append
        powershell -Command "(Get-Content '!GLOBAL_CLAUDE_MD!' -Raw) -replace '(?s)# --- Mobile Team Agent Instructions ---.*?# --- End Mobile Team Agent Instructions ---', '' | Set-Content '!GLOBAL_CLAUDE_MD!.tmp'"
    ...
    echo # --- Mobile Team Agent Instructions --- >> "!GLOBAL_CLAUDE_MD!"
    type "!CLAUDE_MD_SOURCE!" >> "!GLOBAL_CLAUDE_MD!"
    echo. >> "!GLOBAL_CLAUDE_MD!"
    echo # --- End Mobile Team Agent Instructions --- >> "!GLOBAL_CLAUDE_MD!"
...
echo     2. Run: claude
echo     3. Say: "invoke mobile-team-agent"
