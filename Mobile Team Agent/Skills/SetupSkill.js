...
class SetupSkill extends BaseSkill {
  ...
  getTools() {
    return [
      {
        name: "invoke_mobile_team",
        description: "Activate the Mobile Team Agent. Shows setup status and next steps.",
        inputSchema: {
          type: "object",
  ...
  async handleTool(name, args, context) {
    ...
    switch (name) {
      case "invoke_mobile_team": {
        ...
        let out = `Mobile Team Agent v${CONST.VERSION} ACTIVATED.\n\n`;
      ...
      case "get_setup_status": {
        ...
        let out = `Mobile Team Agent v${CONST.VERSION} — Setup Status\n\n`;
