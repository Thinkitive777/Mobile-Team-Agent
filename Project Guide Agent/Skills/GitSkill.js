...
/// file-level diff stats, work area analysis, and per-commit deep dives.
  ...
  async handleTool(name, args, context) {
    ...
    switch (name) {
      case "get_recent_commits": {
        ...
        const commits = includeDiffs
          ? await GitUtils.getCommitsWithDiffs(since, getRepoPath())
          : await GitUtils.getRecentCommits(since, getRepoPath());
        ...
        if (includeAreas) {
          try {
            ...
            out += `\n(Work area analysis unavailable: ${areaErr.message})\n`;
          }
        }
      ...
      case "get_commit_details": {
        ...
        if (!hash || !hash.trim()) {
          return this.errorResponse('commit_hash is required.');
        }
        ...
        // Extract ticket IDs referenced anywhere in the diff
